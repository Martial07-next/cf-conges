...
function ensureConfigured() {
  ...
  if (!process.env.VAPID_PUBLIC_KEY || !process.env.VAPID_PRIVATE_KEY) {
    console.error("Push non envoyé : VAPID_PUBLIC_KEY ou VAPID_PRIVATE_KEY absente côté serveur (à ajouter sur Vercel, en plus de NEXT_PUBLIC_VAPID_PUBLIC_KEY).");
    return false;
  }
  webpush.setVapidDetails(
    process.env.VAPID_SUBJECT || "mailto:admin@cf-reseaux.fr",
    ...
    subs.map(async (sub) => {
      try {
        ...
        if (e.statusCode === 404 || e.statusCode === 410) {
          ...
          console.error(`Push send error (non bloquant) — statusCode ${e.statusCode || "?"} — ${e.body || e.message}`);
        }
      }
    })
  );
