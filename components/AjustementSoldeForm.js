...
export default function AjustementSoldeForm({ users }) {
  ...
  async function envoyer(e) {
    ...
    const res = await fetch("/api/admin/ajustements", {
      ...
      body: JSON.stringify({ userId, annee, montant, motif }),
    });
  ...
  return (
    <Card className="p-6">
      ...
      <p className="text-sm text-brand-dark/60 mb-4">
        Toujours un ajout (+ ou −) tracé avec motif — jamais un écrasement du solde calculé.
      </p>
      <form onSubmit={envoyer} className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="block text-[11px] font-semibold text-brand-dark/60 mb-1">Montant (+ ou −)</label>
            <input type="number" step="0.01" required value={montant} onChange={(e) => setMontant(e.target.value)} placeholder="ex: -2.5" className="w-full px-3 py-2 rounded-lg border border-black/10 bg-brand-cream/60 text-sm" />
          </div>
        </div>
        <div>
