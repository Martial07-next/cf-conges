  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(
          data.error ||
            `Erreur lors de l'enregistrement (${res.status}).`
        );
        return;
      }
      ...
      console.error(
        "Erreur enregistrement solde initial :",
        error
      );
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <p className="font-bold text-brand-dark mb-1">
            Faites une dernière étape
          </p>

          <p className="text-sm text-brand-dark/70 mb-3">
            Pour que votre solde de congés payés soit exact, indiquez
            combien de jours de CP il vous reste actuellement (d'après
            votre dernière fiche de paie par exemple). L'application
            les créditera ensuite automatiquement, à raison de 2,5
            jours le 1er de chaque mois.
          </p>
          ...
          {error && (
            <p className="text-sm text-alert-soft mt-2">
              {error}
            </p>
          )}
        </div>
